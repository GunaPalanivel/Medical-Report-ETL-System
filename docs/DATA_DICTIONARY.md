...
This document explains the two critical data files generated by the ETL pipeline: `patient_metadata.json` and `id_map.json`. For technical details on how these are generated, see [FEATURE_ANALYSIS.md](FEATURE_ANALYSIS.md).
...
2. **Researcher** analysis:
   - "Find all patients with BMI > 30" → Query `patient_metadata.json`.
